...
int main(int argc, char* argv[]) {
    ...
    for (int matid : matids) {
        ...
        SpinerEOSDependsRhoSieTransformable<AllTransform> eos_rs(sp5file, matid); //eventually make it SpinerEOSDependsRhoSie<NullTransfom>, for example
	EOSPAC eos_ref(matid); 
        ...
        for (int rep = 0; rep < ntimes; ++rep) {
                ...
                for (auto& row : P_rs_T) std::fill(row.begin(), row.end(), 0.0);

                for (auto& row : P_eos_ref_sie) std::fill(row.begin(), row.end(), 0.0);
                for (auto& row : P_rt_sie) std::fill(row.begin(), row.end(), 0.0);
                for (auto& row : P_rs_sie) std::fill(row.begin(), row.end(), 0.0);

                for (auto& row : T_back_eos_ref) std::fill(row.begin(), row.end(), 0.0);
                for (auto& row : T_back_rt) std::fill(row.begin(), row.end(), 0.0);
                for (auto& row : T_back_rs) std::fill(row.begin(), row.end(), 0.0);
    ...
    write_matrix_csv((base_output_path / filename).string(), P_eos_ref_T);

    filename = "P_rt_T_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rt_T);

    filename = "P_rs_T_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rs_T);
    ...
    write_matrix_csv((base_output_path / filename).string(), P_eos_ref_sie);

    filename = "P_rt_sie_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rt_sie);

    filename = "P_rs_sie_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rs_sie);
