...
void write_matrix_csv(const std::string &filename, const std::vector<Real> &flat_matrix,
...
// purposes), as well as time trials ==
...
int main(int argc, char *argv[]) {
  ...
  for (int matid : matids) {
    ...
    for (int i = 0; i < nRho; ++i)
      rhos[i] = rhoBounds.i2lin(i);
    for (int j = 0; j < nT; ++j)
      temps[j] = TBounds.i2lin(j);

    // == This will save the values of rho and T corresponding to the indices chosen ==
    std::string rho_file = (base_output_path / ("rho_axis_" + std::to_string(matid) +
                                                "_nRho-" + std::to_string(nRho) + ".csv"))
                               .string();
    std::string temp_file = (base_output_path / ("temp_axis_" + std::to_string(matid) +
                                                 "_nT-" + std::to_string(nT) + ".csv"))
                                .string();
    ...
    std::vector<double> time_T_back_eos_ref_list, time_T_back_rt_list,
        time_T_back_rs_list;

    // These matrices will be filled with the EOS values (overwriting for each trail)
    // Then at the end of the program, we export the matrix to a .csv to later use for
    // accuracy tests
    ...
    for (int rep = 0; rep < ntimes; ++rep) {
      ...
      for (int i = 0; i < nRho;
           ++i) { // replace these with a single for loop that loops through (i,j)
        for (int j = 0; j < nT; ++j) {
          sie_eos_ref[i * nT + j] =
              eos_ref.InternalEnergyFromDensityTemperature(rhos[i], temps[j]);
        }
      }
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) {
          P_eos_ref_T[i * nT + j] =
              eos_ref.PressureFromDensityTemperature(rhos[i], temps[j]);
        }
      }
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT;
             ++j) { // is this line supposed to still be nT, it uses sie_eos_ref(i,j)
          P_eos_ref_sie[i * nT + j] =
              eos_ref.PressureFromDensityInternalEnergy(rhos[i], sie_eos_ref[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) { // same with this one
          T_back_eos_ref[i * nT + j] = eos_ref.TemperatureFromDensityInternalEnergy(
              rhos[i], sie_eos_ref[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) {
          sie_rt[i * nT + j] =
              eos_rt.InternalEnergyFromDensityTemperature(rhos[i], temps[j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) { // same with this one
          P_rt_sie[i * nT + j] =
              eos_rt.PressureFromDensityInternalEnergy(rhos[i], sie_rt[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) { // same with this one
          T_back_rt[i * nT + j] =
              eos_rt.TemperatureFromDensityInternalEnergy(rhos[i], sie_rt[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) {
          sie_rs[i * nT + j] =
              eos_rs.InternalEnergyFromDensityTemperature(rhos[i], temps[j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) { // same with this one
          P_rs_sie[i * nT + j] =
              eos_rs.PressureFromDensityInternalEnergy(rhos[i], sie_rs[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
      ...
      for (int i = 0; i < nRho; ++i) {
        for (int j = 0; j < nT; ++j) { // same with this one
          T_back_rs[i * nT + j] =
              eos_rs.TemperatureFromDensityInternalEnergy(rhos[i], sie_rs[i * nT + j]);
        }
      }
      t1 = high_resolution_clock::now();
      elapsed = t1 - t0;
    ...
    // Only save the filled in grid values on the last loop (only one grid exported to
    // compare accuracies)
    std::string filename;

    // internal energy grids
    filename = "sie_eos_ref_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), sie_eos_ref, nRho, nT);

    filename = "sie_rt_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), sie_rt, nRho, nT);

    filename = "sie_rs_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), sie_rs, nRho, nT);

    // pressure from density & temperature grids
    filename = "P_eos_ref_T_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_eos_ref_T, nRho, nT);

    filename = "P_rt_T_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rt_T, nRho, nT);

    filename = "P_rs_T_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rs_T, nRho, nT);

    // pressure from density & internal energy grids
    filename = "P_eos_ref_sie_" + std::to_string(matid) + "_nRho-" +
               std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_eos_ref_sie, nRho, nT);

    filename = "P_rt_sie_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rt_sie, nRho, nT);

    filename = "P_rs_sie_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), P_rs_sie, nRho, nT);

    // temperature grids
    filename = "T_back_eos_ref_" + std::to_string(matid) + "_nRho-" +
               std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), T_back_eos_ref, nRho, nT);

    filename = "T_back_rt_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), T_back_rt, nRho, nT);

    filename = "T_back_rs_" + std::to_string(matid) + "_nRho-" + std::to_string(nRho) +
               "_nT-" + std::to_string(nT) + ".csv";
    write_matrix_csv((base_output_path / filename).string(), T_back_rs, nRho, nT);

    // after creating this .csv, should I average the last 15, can you explain again why
    // the first 5 may be bad
    std::string timing_file =
        (base_output_path / ("timing_" + std::to_string(matid) + "_nRho-" +
                             std::to_string(nRho) + "_nT-" + std::to_string(nT) + ".csv"))
            .string();
    std::ofstream timing_out(timing_file);

    timing_out << "sie_eos_ref,sie_rt,sie_rs,P_eos_ref_T,P_rt_T,P_rs_T,P_eos_ref_sie,P_"
                  "rt_sie,P_rs_sie,T_back_eos_ref,T_back_rt,T_back_rs\n";

    for (size_t i = 0; i < time_sie_eos_ref_list.size(); ++i) {
      timing_out << time_sie_eos_ref_list[i] << "," << time_sie_rt_list[i] << ","
                 << time_sie_rs_list[i] << "," << time_P_eos_ref_T_list[i] << ","
                 << time_P_rt_T_list[i] << "," << time_P_rs_T_list[i] << ","
                 << time_P_eos_ref_sie_list[i] << "," << time_P_rt_sie_list[i] << ","
                 << time_P_rs_sie_list[i] << "," << time_T_back_eos_ref_list[i] << ","
                 << time_T_back_rt_list[i] << "," << time_T_back_rs_list[i] << "\n";
    }
