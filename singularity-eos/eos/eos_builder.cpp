...
EOS EOSBuilder::buildEOS(EOSBuilder::EOSType type, EOSBuilder::params_t base_params,
  ...
  if (type == EOSType::SNLGruneisen) {
    ...
    Real P0 = mpark::get<Real>(base_params["P0"]);
    Real Cv = mpark::get<Real>(base_params["Cv"]);
    return SNLGruneisen(C0, s1, s2, s3, G0, b, rho0, T0, P0, Cv);
  }
  if (type == EOSType::Vinet) {
    ...
    Real d2 = mpark::get<Real>(base_params["d2"]);
    Real d3 = mpark::get<Real>(base_params["d3"]);
    return Vinet(rho0, T0, B0, BP0, A0, Cv0, E0, S0, d2, d3);
  }
  if (type == EOSType::JWL) {
