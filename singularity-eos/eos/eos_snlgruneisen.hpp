...
#ifndef _SINGULARITY_EOS_EOS_SNLGRUNEISEN_HPP_
#define _SINGULARITY_EOS_EOS_SNLGRUNEISEN_HPP_
...
// EOS created at Sandia National Laboratories by Allen C. Robinson. 
// "The Mie-Grüneisen Power Equation of State", SAND2019-6025
// It is particularly suited for high-pressure applications.
class SNLGruneisen : public EosBase<SNLGruneisen> {
 public:
  SNLGruneisen() = default;
  PORTABLE_INLINE_FUNCTION
  SNLGruneisen(const Real C0, const Real s1, const Real s2, const Real s3, const Real G0,
  ...
  SNLGruneisen GetOnDevice() { return *this; }
  ...
  SG_ADD_BASE_CLASS_USINGS(SNLGruneisen)
  PORTABLE_INLINE_FUNCTION
  int nlambda() const noexcept { return 0; }
  static constexpr unsigned long PreferredInput() { return _preferred_input; }
  PORTABLE_INLINE_FUNCTION void PrintParams() const {
    static constexpr char s1[]{"SNL Gruneisen Params: "};
  ...
  static std::string EosType() { return std::string("SNLGruneisen"); }
  static std::string EosPyType() { return EosType(); }

 private:
  Real _C0, _s1, _s2, _s3, _G0, _b, _rho0, _T0, _P0, _Cv, _rho_max;
  // static constexpr const char _eos_type[] = {"SNLGruneisen"};
  PORTABLE_INLINE_FUNCTION
...
*/
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::ComputeRhoMax(const Real s1, const Real s2,
                                                       const Real s3, const Real rho0) {
  ...
  if (is_near_zero(s2, EPS) && is_near_zero(s3, EPS) && s1 > 1.) {
    ...
    if (poly(minbound) * poly(maxbound) < 0.) {
      ...
      if (status != Status::SUCCESS) {
        ...
        EOS_ERROR("SNLGruneisen initialization: Cubic root find failed. Maximum "
                  "density cannot be"
...
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::dPres_drho_e(const Real rho_in,
...
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::PressureFromDensityInternalEnergy(
...
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::BulkModulusFromDensityInternalEnergy(
    const Real rho_in, const Real sie, Real *lambda) const {
  using namespace gruneisen_utils;
  const Real rho = std::min(rho_in, _rho_max);
...
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::PressureFromDensityTemperature(
...
PORTABLE_INLINE_FUNCTION Real SNLGruneisen::BulkModulusFromDensityTemperature(
    const Real rho_in, const Real temp, Real *lambda) const {
  const Real rho = std::min(rho_in, _rho_max);
  return BulkModulusFromDensityInternalEnergy(
      rho, InternalEnergyFromDensityTemperature(rho, temp));
}
PORTABLE_INLINE_FUNCTION void SNLGruneisen::DensityEnergyFromPressureTemperature(
  ...
  if (press < Pref) {
    ...
    if (status != Status::SUCCESS) {
      // Root finder failed even though the solution was bracketed... this is an error
      EOS_ERROR("SNLGruneisen::DensityEnergyFromPressureTemperature: "
                "Root find failed to find a solution given P, T\n");
    }
  }
}
PORTABLE_INLINE_FUNCTION void SNLGruneisen::FillEos(Real &rho_in, Real &temp, Real &sie,
  ...
  if (thermalqs::temperature & input && thermalqs::pressure & input) {
    ...
    EOS_ERROR("SNLGruneisen FillEos: Density and energy are currently required inputs "
...
PORTABLE_INLINE_FUNCTION
void SNLGruneisen::ValuesAtReferenceState(Real &rho, Real &temp, Real &sie, Real &press,
...
#endif // _SINGULARITY_EOS_EOS_SNLGRUNEISEN_HPP_
