...
static struct ThermalUnitsInit { } thermal_units_init_tag;
...
public:
  ...
  Real TemperatureFromDensityInternalEnergy(const Real rho, const Real sie,
    ...
    const Real temp = t_.TemperatureFromDensityInternalEnergy(rho*rho_unit_,
							      sie*sie_unit_,
							      lambda);
  ...
  Real InternalEnergyFromDensityTemperature(const Real rho, const Real temperature,
					    Real *lambda = nullptr) const {
    const Real sie = t_.InternalEnergyFromDensityTemperature(rho*rho_unit_,
							     temperature*temp_unit_,
							     lambda);
  ...
  Real PressureFromDensityInternalEnergy(const Real rho, const Real sie,
					 Real *lambda = nullptr) const {
    const Real P = t_.PressureFromDensityInternalEnergy(rho*rho_unit_,
							sie*sie_unit_,
							lambda);
  ...
  Real BulkModulusFromDensityInternalEnergy(const Real rho, const Real sie,
					    Real *lambda = nullptr) const {
    const Real bmod = t_.BulkModulusFromDensityInternalEnergy(rho*rho_unit_,
							      sie*sie_unit_,
							      lambda);
  ...
  Real PressureFromDensityTemperature(const Real rho, const Real temp,
				      Real *lambda = nullptr) const {
    const Real P = t_.PressureFromDensityTemperature(rho*rho_unit_,
						     temp*temp_unit_,
						     lambda);
    return inv_press_unit_*P;
  }
  ...
  Real SpecificHeatFromDensityTemperature(const Real rho, const Real temp,
					  Real *lambda = nullptr) const {
    const Real cv = t_.SpecificHeatFromDensityTemperature(rho*rho_unit_,
							  temp*temp_unit_,
							  lambda);
    return inv_cv_unit_*cv;
  }
  ...
  Real BulkModulusFromDensityTemperature(const Real rho, const Real temp,
					 Real *lambda = nullptr) const {
    const Real bmod = t_.BulkModulusFromDensityTemperature(rho*rho_unit_,
							   temp*temp_unit_,
							   lambda);
    return inv_bmod_unit_*bmod;
  }
  ...
  PORTABLE_FUNCTION
  void PTofRE(const Real rho, const Real sie, Real *lambda,
	      Real &press, Real &temp,
	      Real &dpdr, Real &dpde, Real &dtdr, Real &dtde) const {
    t_.PTofRE(rho*rho_unit_, sie*sie_unit_, lambda,
	      press, temp, dpdr, dpde, dtdr, dtde);
