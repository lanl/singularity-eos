...
class SAPRampEOS : public EosBase<SAPRampEOS<T>> {
 public:
  ...
  SAPRampEOS(T &&t, const Real shift) : t_(std::forward<T>(t)), shift_(shift) {}
  SAPRampEOS() = default;
  ...
  Real get_ramp_pressure(Real rho) const {
    const Real p_ramp {rho < rholow ? 0.0              :
                       rho < rhomid ? a*(rho/r0 - 1.0) :
                                      b*(rho/r0 - c)};
    return p_ramp;
  }
  ...
  void FillEos(Real &rho, Real &temp, Real &energy, Real &press, Real &cv, Real &bmod,
    ...
    case thermalqs::density | thermalqs::specific_internal_energy:
      ...
      break;
    default:
      EOS_ERROR("Didn't find a valid input for SAPRampEOS::FillEOS\n");
    }
  }
