...
template <typename T>
auto makeUnitSystem(T &&eos, bool use_length_time, Real rho_unit, Real sie_unit,
...
template <typename T>
auto makeRelativistic(T &&eos, Real cl) {
  return RelativisticEOS<T>(std::move(eos), cl);
}

template <typename T>
auto makeBilinearRamp(T &&eos, Real r0, Real a, Real b, Real c) {
  return BilinearRampEOS<T>(std::forward<T>(eos), r0, a, b, c);
}

template <typename T>
auto applyShiftAndScale(T &&eos, bool scaled, bool shifted, Real scale, Real shift) {
  if (shifted && scaled) {
...
auto applyWrappedShiftAndScale(T &&eos, bool scaled, bool shifted, Real scale, Real shift,
                               ARGS... args) {
  if (shifted && scaled) {
    ShiftedEOS<T> a(std::forward<T>(eos), shift);
    ScaledEOS<ShiftedEOS<T>> b(std::move(a), scale);
...
template <typename T>
auto applyShiftAndScaleAndBilinearRamp(T &&eos, bool scaled, bool shifted, bool ramped,
