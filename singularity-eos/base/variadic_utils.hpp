...
struct flatten_helper<type_list<Ts...>,
                      type_list<type_list<Heads...>,
                      Tail...>
                     >
{
  using type = typename flatten_helper<type_list<Ts...>,
...
struct flatten_helper<type_list<Ts...>,
                      type_list<Head, Tail...>
                     > {
    using type = typename flatten_helper<type_list<Ts..., Head>,
                                         type_list<Tail...>>::type;
};
...
template < template <typename> class Template, typename T >
struct is_not_duplicate_nested< Template,
                                Template< Template<T> > > : std::false_type {};

// filter
template <template <typename> class,
          template < template<typename> class, class> class,
...
struct filter<ADAPTER, Pred, Variadic, T, Ts...>
{
  ...
  using type = typename std::conditional<
    ...
    typename Cons<T, typename filter<ADAPTER, Pred,
                                     Variadic, Ts...>::type>::type,
...
constexpr auto filter_nested(type_list<Ts...>) {
  using f_list = typename filter<U, is_not_duplicate_nested, type_list,
                                 Ts...>::type;
  return f_list{};
}
...
template < template<typename> class FIRST, template<typename> class ... Us,
           typename ... Ts>
constexpr auto filter_nested_variadic(adapt_list<FIRST, Us...> m,
                                      type_list<Ts...> l)
{
