...
#endif
  // Eigen VERSION
  Eigen::Map<Eigen::Matrix<Real, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> A(a,n,n);
...
class CacheAccessor {
 public:
  ...
  Real * operator[](const int m) const {
    return cache_ + m*MAX_NUM_LAMBDAS;
  }
 private:
  Real *cache_;
};
...
class PTESolverBase {
 public:
  ...
  virtual void Fixup() const {
    Real vsum = 0;
    for (int m = 0; m < nmat; ++m) vsum += vfrac[m];
    for (int m = 0; m < nmat; ++m) vfrac[m] *= vfrac_total/vsum;
  }
  PORTABLE_INLINE_FUNCTION
  void Finalize() {
  ...
  PORTABLE_INLINE_FUNCTION
  bool Solve() const {
    for (int m = 0; m < neq; m++) dx[m] = residual[m];
 ...
 protected:
  ...
  PTESolverBase(int nmats, int neqs, EOSIndexer &eos_, const Real vfrac_tot,
                ...
                Real *&scratch)
    : nmat(nmats), neq(neqs), niter(0),
      eos(eos_),
      vfrac_total(vfrac_tot),
      sie_total(sie_tot),
      rho(rho_),
      vfrac(vfrac_),
      sie(sie_),
  ...
  void InitBase() {
    Real Tguess = 0.0;
    for (int m = 0; m < nmat; ++m) Tguess = std::max(Tguess, temp[m]);
    Tnorm = Tguess;
    ...
    for (int m = 0; m < nmat; m++) {
      ...
      if (eos[m].PreferredInput() ==
        ...
        press[m] = eos[m].PressureFromDensityInternalEnergy(rho[m], sie[m], Cache[m]) / u_total;
  ...
  Real ResidualNorm() const {
    Real norm = 0.0;
    for (int m = 0; m < neq; m++) norm += residual[m]*residual[m];
    return 0.5*norm;
  }

  PORTABLE_FORCEINLINE_FUNCTION
  int MatIndex(const int &i, const int &j) const {
    return i*neq + j;
  }
  ...
  void TryIdealPTE(T *solver) {
    ...
    for (int m = 0; m < nmat; ++m) {
      ...
      if (rhobar[m] / vfrac[m] < eos[m].RhoPmin(Tnorm * Tideal)) {
        ...
        for (int n = m; n >= 0; n--) vfrac[n] = vtemp[n];
        return;
      }
    }
    ...
    for (int m = 0; m < nmat; ++m) {
      rho[m] = rhobar[m] / vfrac[m];
      sie[m] = eos[m].InternalEnergyFromDensityTemperature(rho[m], Tnorm * Tideal, Cache[m]);
      if (eos[m].PreferredInput() ==
          (thermalqs::density | thermalqs::specific_internal_energy)) {
        press[m] = eos[m].PressureFromDensityInternalEnergy(rho[m], sie[m], Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() ==
                (thermalqs::density | thermalqs::temperature)) {
        press[m] = eos[m].PressureFromDensityTemperature(rho[m], Tnorm * Tideal, Cache[m]) / u_total;
      }
    }
    // fill in the residual
    solver->Residual();
    Real res_norm_new = 0.0;
    for (int m = 0; m < neq; m++) res_norm_new += residual[m] * residual[m];
    ...
    if (res_norm_new > res_norm_old) {
      ...
      for (int m = 0; m < neq; ++m) residual[m] = res[m];
    } else {
...
class PTESolverRhoT : public mix_impl::PTESolverBase<EOSIndexer, RealIndexer> {
 ...
 public:
  ...
  PORTABLE_INLINE_FUNCTION
  PTESolverRhoT(const int nmat, EOS_t &&eos, const Real vfrac_tot,
                const Real sie_tot, Real_t &&rho, Real_t &&vfrac,
                Real_t &&sie, Real_t &&temp, Real_t &&press,
                Lambda_t &&lambda, Real *scratch)
    : mix_impl::PTESolverBase<EOSIndexer, RealIndexer>(nmat, nmat+1, eos, vfrac_tot, sie_tot, rho, vfrac, sie, temp, press, scratch) {
  ...
  bool CheckPTE() const {
    ...
    bool converged_u =  (error_u < pte_rel_tolerance_e
                      || error_u < pte_abs_tolerance_e);
  ...
  void Jacobian() const {
    ...
    for (int m = 0; m < nmat; m++) {
      ...
      Real p_pert;
      Real e_pert = eos[m].InternalEnergyFromDensityTemperature(rho_pert, Tnorm * Tequil, Cache[m]);
      if (eos[m].PreferredInput() == (thermalqs::density | thermalqs::specific_internal_energy)) {
        p_pert = eos[m].PressureFromDensityInternalEnergy(rho_pert, e_pert, Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() == (thermalqs::density | thermalqs::temperature)) {
        p_pert = eos[m].PressureFromDensityTemperature(rho_pert, Tnorm * Tequil, Cache[m]) / u_total;
      }
      ...
      e_pert = eos[m].InternalEnergyFromDensityTemperature(rho[m], Tnorm * (Tequil + dT), Cache[m]);
      if (eos[m].PreferredInput() == (thermalqs::density | thermalqs::specific_internal_energy)) {
        p_pert = eos[m].PressureFromDensityInternalEnergy(rho[m], e_pert, Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() == (thermalqs::density | thermalqs::temperature)) {
        p_pert = eos[m].PressureFromDensityTemperature(rho[m], Tnorm * (Tequil + dT), Cache[m]) / u_total;
      }
  ...
  Real ScaleDx() const {
    ...
    for (int m = 0; m < nmat; m++) {
      const Real rho_min = std::max(eos[m].RhoPmin(Tnorm * Tequil),
                                    eos[m].RhoPmin(Tnorm * Tnew));
    ...
    for (int i = 0; i < neq; ++i) dx[i] *= scale;
    return scale;
  }
  ...
  Real TestUpdate(const Real scale) {
    if (scale == 1.0) {
      Ttemp = Tequil;
      for (int m = 0; m < nmat; ++m) vtemp[m] = vfrac[m];
    }
    Tequil = Ttemp + scale * dx[nmat];
    for (int m = 0; m < nmat; ++m) {
      vfrac[m] = vtemp[m] + scale * dx[m];
      rho[m] = rhobar[m] / vfrac[m];
      u[m] = rhobar[m] * eos[m].InternalEnergyFromDensityTemperature(rho[m], Tnorm * Tequil, Cache[m]);
      sie[m] = u[m] / rhobar[m];
      u[m] /= u_total;
      temp[m] = Tequil;
      if (eos[m].PreferredInput() ==
        ...
        press[m] = eos[m].PressureFromDensityInternalEnergy(rho[m], sie[m], Cache[m]) / u_total;
                ...
                (thermalqs::density | thermalqs::temperature)) {
        press[m] = eos[m].PressureFromDensityTemperature(rho[m], Tnorm * Tequil, Cache[m]) / u_total;
      }
    }
    Residual();
    return ResidualNorm();
...
class PTESolverRhoU : public mix_impl::PTESolverBase<EOSIndexer, RealIndexer> {
 ...
 public:
  ...
  template <typename EOS_t, typename Real_t, typename Lambda_t>
  PORTABLE_INLINE_FUNCTION
  PTESolverRhoU(const int nmat, EOS_t &&eos, const Real vfrac_tot,
                const Real sie_tot, Real_t &&rho, Real_t &&vfrac,
                Real_t &&sie, Real_t &&temp, Real_t &&press,
                Lambda_t &&lambda, Real *scratch)
    : mix_impl::PTESolverBase<EOSIndexer,RealIndexer>(nmat, 2*nmat, eos, vfrac_tot, sie_tot, rho, vfrac, sie, temp, press, scratch) {
    dpdv = AssignIncrement(scratch, nmat);
  ...
  void Jacobian() const {
    ...
    for (int m = 0; m < nmat; m++) {
      ...
      if (eos[m].PreferredInput() ==
          (thermalqs::density | thermalqs::specific_internal_energy)) {
        p_pert = eos[m].PressureFromDensityInternalEnergy(rho_pert, sie[m], Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() ==
                (thermalqs::density | thermalqs::temperature)) {
        p_pert = eos[m].PressureFromDensityTemperature(rho_pert, Tnorm * t_pert, Cache[m]) / u_total;
      }
      dpdv[m] = (p_pert - press[m]) / dv;
      ...
      t_pert =
          eos[m].TemperatureFromDensityInternalEnergy(rho[m], u_total * e_pert, Cache[m]) / Tnorm;
      if (eos[m].PreferredInput() ==
          (thermalqs::density | thermalqs::specific_internal_energy)) {
        p_pert = eos[m].PressureFromDensityInternalEnergy(rho[m], u_total * e_pert, Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() ==
                (thermalqs::density | thermalqs::temperature)) {
        p_pert = eos[m].PressureFromDensityTemperature(rho[m], Tnorm * t_pert, Cache[m]) / u_total;
      }
  ...
  Real ScaleDx() const {
    ...
    for (int m = 0; m < nmat; ++m) {
      ...
      const Real rho_min = std::max(eos[m].RhoPmin(Tnorm * temp[m]),
                                    eos[m].RhoPmin(Tnorm * tt));
      const Real alpha_max = rhobar[m] / rho_min;
      if (scale * dx[m] > 0.5 * (alpha_max - vfrac[m])) {
        scale = 0.5 * (alpha_max - vfrac[m]) / dx[m];
      }
  ...
  Real TestUpdate(const Real scale) const {
    ...
    for (int m = 0; m < nmat; ++m) {
      ...
      temp[m] = eos[m].TemperatureFromDensityInternalEnergy(rho[m], sie[m], Cache[m]) / Tnorm;
      if (eos[m].PreferredInput() ==
          (thermalqs::density | thermalqs::specific_internal_energy)) {
        press[m] = eos[m].PressureFromDensityInternalEnergy(rho[m], sie[m], Cache[m]) / u_total;
      } else if (eos[m].PreferredInput() ==
                (thermalqs::density | thermalqs::temperature)) {
        press[m] = eos[m].PressureFromDensityTemperature(rho[m], Tnorm * temp[m], Cache[m]) / u_total;
      }
    }
    Residual();
    return ResidualNorm();
  ...
  for (niter = 0; niter < pte_max_iter; ++niter) {
    ...
    if (err > err_old + line_search_alpha * gradfdx) {
      ...
      for (int line_iter = 0; line_iter < line_search_max_iter; line_iter++) {
        err = s.TestUpdate(scale);
        if (err < err_old + line_search_alpha * scale * gradfdx)
          break;
        scale *= line_search_fac;
      }
    }
