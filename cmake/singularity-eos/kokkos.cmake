...
macro(_singularity_handle_custom_kokkos_tpl)
  ...
  if(NOT ("${KTPL_NAME}" STREQUAL "Kokkos" OR "${KTPL_NAME}" STREQUAL
                                              "KokkosKernels"))
    message(
      FATAL_ERROR
        "Custom TPL only supported for \"Kokkos\" and \"KokkosKernels\" (got \"${KTPL_NAME}\")"
    )
  endif()
  ...
  if(NOT SINGULARITY_CUSTOM_${tplUp}_DIR)
    message(
      FATAL_ERROR
        "Selected custom ${KTPL_NAME} source, but SINGULARITY_CUSTOM_${tplUp}_DIR not set."
        "Set and reconfigure, or disable custom source.")
  else()
    if(NOT EXISTS ${SINGULARITY_CUSTOM_${tplUp}_DIR})
      message(
        FATAL_ERROR
          "Custom ${KTPL_NAME} source dir ${SINGULARITY_CUSTOM_${tplUP}_DIR} does not exist."
      )
    endif()
  endif()
  if(NOT SINGULARITY_CUSTOM_${tplUp}_CACHE)
    message(
      WARNING
        "Selected custom ${KTPL_NAME} source, but SINGULARITY_CUSTOM_${tplUp}_CACHE not set."
        " CMake will configure with default options."
        " See ${PROJECT_SOURCE_DIR}/utils/scripts/caches for sample caches.")
    set(SINGULARITY_CUSTOM_${tplUp}_CACHE
        "${PROJECT_SOURCE_DIR}/utils/scripts/caches/${tplLo}_default.cmake")
  endif()
  if(NOT EXISTS ${SINGULARITY_CUSTOM_${tplUp}_CACHE})
    message(
      FATAL
      "Custom kokkos CMake cache file ${SINGULARITY_CUSTOM_${tplUp}_CACHE} does not exist."
    )
  endif()
  include(${SINGULARITY_CUSTOM_${tplUp}_CACHE})

  if(NOT TARGET Kokkos::${tplLo})
    add_subdirectory(${SINGULARITY_CUSTOM_${tplUp}_DIR}
                     ${PROJECT_BINARY_DIR}/_deps/${tplLo}-build)
  else()
    message(
      WARNING
        "${KTPL_NAME} target already present, not using custom source directory."
    )
  endif()
