...
public:
  ...
  auto GetOnDevice() {
    return ScaledEOS<T>(t_.GetOnDevice(), scale_);
  }
  ...
  PORTABLE_FUNCTION
  Real SpecificHeatFromDensityInternalEnergy(const Real rho, const
                                             Real sie,
                                             Real *lambda=nullptr) const {
  ...
  PORTABLE_FUNCTION
  Real PressureFromDensityTemperature(const Real rho,
                                      const Real temperature,
                                      Real *lambda=nullptr) const {
  ...
  void PTofRE(const Real rho, const Real sie,
      ...
      t_.PTofRE(scale_*rho,inv_scale_*sie,lambda,press,temp,dpdr,dpde,dtdr,dtde);
      dpdr = dpdr*scale_;
      dtdr = dtdr*scale_;
...
public:
  // move semantics ensures dynamic memory comes along for the ride
  ShiftedEOS(T&& t, const Real shift)
    : t_(std::forward<T>(t)) 
    , shift_(shift)
  {}
  ...
  auto GetOnDevice() {
    return ShiftedEOS<T>(t_.GetOnDevice(), shift_);
  }
  inline void Finalize() { t_.Finalize(); }
  ...
  Real TemperatureFromDensityInternalEnergy(const Real rho, const Real sie,
                                            Real* lambda=nullptr) const {
    return t_.TemperatureFromDensityInternalEnergy(rho, sie - shift_,
                                                  lambda);
  }
  PORTABLE_FUNCTION
  Real InternalEnergyFromDensityTemperature(const Real rho,
...
public:
  ...
  RelativisticEOS(T&& t, const Real cl)
    : t_(std::forward<T>(t))
    , cl_(cl) // speed of light, units arbitrary
  ...
  PORTABLE_FUNCTION RelativisticEOS& operator=(const RelativisticEOS&) = default;
  PORTABLE_FUNCTION RelativisticEOS(const RelativisticEOS&) = default;
  
  auto GetOnDevice() {
    return RelativisticEOS<T>(t_.GetOnDevice(), cl_);
  }
  inline void Finalize() { t_.Finalize(); }
...
public:
  ...
  PORTABLE_FUNCTION IdealGas(Real gm1, Real Cv)
    : _Cv(Cv),
      _gm1(gm1),
      _rho0(_P0/(_gm1*_Cv*_T0)),
      _sie0(_Cv*_T0),
      _bmod0((_gm1+1)*_gm1*_rho0*_Cv*_T0),
      _dpde0(_gm1*_rho0),
      _dvdt0(1./(_rho0*_T0))
  { }

  IdealGas GetOnDevice() { return *this; }
  PORTABLE_FUNCTION Real TemperatureFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
  PORTABLE_FUNCTION Real InternalEnergyFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
  PORTABLE_FUNCTION Real PressureFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
  PORTABLE_FUNCTION Real PressureFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
  PORTABLE_FUNCTION Real SpecificHeatFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
  PORTABLE_FUNCTION Real SpecificHeatFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real BulkModulusFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real BulkModulusFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real GruneisenParamFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real GruneisenParamFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION void FillEos(Real& rho, Real& temp, Real& energy, Real& press, Real& cv, Real& bmod, const unsigned long output, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION
    void ValuesAtReferenceState(Real& rho, Real& temp, Real& sie, Real& press,
                                Real& cv, Real& bmod, Real& dpde, Real& dvdt,
                                Real *lambda=nullptr) const;
    ...
    PORTABLE_FUNCTION void DensityEnergyFromPressureTemperature(const Real press, const Real temp, Real * lambda, Real& rho, Real& sie) const;
  ...
  private:
    ...
    static constexpr const unsigned long _preferred_input = thermalqs::density|thermalqs::specific_internal_energy;
};

// COMMENT: This is meant to be an implementation of the Steinberg version of the Gruneisen EOS which
// should correspond to eostype(3) in xRAGE and /[...]/eos/gruneisen in FLAG
class Gruneisen {
  public:
  ...
  PORTABLE_FUNCTION Gruneisen(const Real C0,
                              const Real s1, const Real s2, const Real s3,
                              const Real G0, const Real b, const Real rho0,
                              const Real T0, const Real P0, const Real Cv):
    _C0(C0),
    _s1(s1), _s2(s2), _s3(s3),
    _G0(G0), _b(b),
    _rho0(rho0), _T0(T0), _P0(P0), _Cv(Cv)
  { }
    ...
    PORTABLE_FUNCTION Real PressureFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real PressureFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real SpecificHeatFromDensityTemperature(const Real rho, const Real temperatummmmmmre, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real SpecificHeatFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real BulkModulusFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real BulkModulusFromDensityInternalEnergy(const Real rho, const Real sie, Real *lambda=nullptr) const;
    PORTABLE_FUNCTION Real GruneisenParamFromDensityTemperature(const Real rho, const Real temperature, Real *lambda=nullptr) const;
...
// COMMENT: This is meant to be an implementation of the "standard" JWL as implemented in xRAGE
// for eostype(1).  It does not include any energy shifting
class JWL {
  public:
    ...
    PORTABLE_FUNCTION JWL(const Real A, const Real B,
        const Real R1, const Real R2,
        const Real w, const Real rho0, const Real Cv) :
        _A(A), _B(B),
        _R1(R1), _R2(R2),
        _w(w), _rho0(rho0), _Cv(Cv) { }
...
class DavisReactants {
  public:
    ...
    PORTABLE_FUNCTION
    DavisReactants(const Real rho0, const Real e0, const Real P0,
                   const Real T0, const Real A, const Real B, const Real C,
                   const Real G0, const Real Z, const Real alpha,
                   const Real Cv0)
    : _rho0(rho0),
      _e0(e0),
      _P0(P0),
  ...
  private:
    ...
    static constexpr unsigned long _preferred_input = thermalqs::density|thermalqs::specific_internal_energy;
...
class DavisProducts {
  public:
    ...
    void ValuesAtReferenceState(Real& rho, Real& temp, Real& sie, Real& press,
                         Real& cv, Real& bmod, Real& dpde, Real& dvdt,
                         Real *lambda=nullptr) const;
    //PORTABLE_FUNCTION void PTofRE(const Real rho, const Real sie, Real * lambda, Real& press, Real& temp, Real & dpdr, Real & dpde, Real & dtdr, Real & dtde) const;
    PORTABLE_INLINE_FUNCTION
    int nlambda() const noexcept { return 0; }
    static constexpr unsigned long PreferredInput() {return _preferred_input;}
    PORTABLE_FUNCTION void PrintParams() const {
      ...
      printf("%sa:%e b:%e k:%e\nn:%e vc:%e pc:%e\nCv:%e E0:%e\n",
             s1, _a, _b, _k, _n, _vc, _pc, _Cv, _E0);
    }
    inline void Finalize() { }
...
public:
  ...
  struct Lambda { enum Index { lRho = 0, lT = 1 }; };
  ...
  PORTABLE_FUNCTION SpinerEOSDependsRhoT& operator=(SpinerEOSDependsRhoT&&) = default;
  PORTABLE_FUNCTION SpinerEOSDependsRhoT& operator=(const SpinerEOSDependsRhoT&) = default;
  PORTABLE_FUNCTION SpinerEOSDependsRhoT(const SpinerEOSDependsRhoT&) = default;

  SpinerEOSDependsRhoT GetOnDevice();

  PORTABLE_FUNCTION
  Real TemperatureFromDensityInternalEnergy(const Real rho,
                                            const Real sie,
                                            Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real InternalEnergyFromDensityTemperature(const Real rho,
  ...
  Real PressureFromDensityTemperature(const Real rho,
                                      ...
                                      Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real PressureFromDensityInternalEnergy(const Real rho,
                                         const Real sie,
                                         Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real SpecificHeatFromDensityTemperature(const Real rho,
                                          ...
                                          Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real SpecificHeatFromDensityInternalEnergy(const Real rho,
                                             const Real sie,
                                             Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real BulkModulusFromDensityTemperature(const Real rho,
                                         ...
                                         Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real BulkModulusFromDensityInternalEnergy(const Real rho,
                                            const Real sie,
                                            Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real GruneisenParamFromDensityTemperature(const Real rho,
                                         ...
                                         Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real GruneisenParamFromDensityInternalEnergy(const Real rho,
                                            const Real sie,
                                            Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  void DensityEnergyFromPressureTemperature(const Real press, const Real temp,
  ...
  PORTABLE_FUNCTION
  void FillEos(Real& rho, Real& temp,
...
private:
  ...
  herr_t loadDataboxes_(const std::string& matid_str, hid_t file,
  ...
  PORTABLE_FUNCTION
  Real lTFromlRhoSie_(const Real lRho, const Real sie,
                      TableStatus& whereAmI,
                      Real* lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real lTFromlRhoP_(const Real lRho, const Real press,
                    TableStatus& whereAmI,
                    Real* lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real lRhoFromPlT_(const Real P, const Real lT,
                    TableStatus& whereAmI,
                    Real* lambda=nullptr) const;
  PORTABLE_FUNCTION
  void getLogsRhoT_(const Real rho, const Real temperature,
                    Real& lRho, Real& lT,
                    Real* lambda=nullptr) const;
  PORTABLE_FUNCTION
  ...
  PORTABLE_FUNCTION
  Real PFromRholRhoTlT_(const Real rho, const Real lRho,
  ...
  Real CvFromlRholT_(const Real lRho, const Real lT,
                     const TableStatus& whereAmI) const;
  PORTABLE_FUNCTION
  Real bModFromRholRhoTlT_(const Real rho, const Real lRho,
                           const Real T, const Real lT,
                           const TableStatus& whereAmI) const;
  PORTABLE_FUNCTION
  ...
  static constexpr const unsigned long _preferred_input = thermalqs::density|thermalqs::temperature;
...
public:
  ...
  { }
  PORTABLE_FUNCTION SpinerEOSDependsRhoSie& operator=(SpinerEOSDependsRhoSie&&) = default;
  PORTABLE_FUNCTION SpinerEOSDependsRhoSie& operator=(const SpinerEOSDependsRhoSie&) = default;
  PORTABLE_FUNCTION SpinerEOSDependsRhoSie(const SpinerEOSDependsRhoSie&) = default;
...
private:
  ...
  herr_t loadDataboxes_(const std::string& matid_str,
                        hid_t file, hid_t lTGroup, hid_t lEGroup);
  ...
  PORTABLE_INLINE_FUNCTION Real fromLog_(const Real lx, const Real offset) const {
    return std::pow(10.,lx) - offset;
  }
  PORTABLE_FUNCTION
  Real interpRhoT_(const Real rho, const Real T,
                   const Spiner::DataBox& db,
                   Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  Real interpRhoSie_(const Real rho, const Real sie,
                     const Spiner::DataBox& db,
                     Real *lambda=nullptr) const;
  PORTABLE_FUNCTION
  ...
  static constexpr unsigned long _preferred_input = thermalqs::density|thermalqs::temperature;
...
public:
  ...
  // A weakly typed index map for lambdas
  struct Lambda { enum Index { Ye = 0, lT = 1 }; };
  ...
  PORTABLE_FUNCTION StellarCollapse& operator=(const StellarCollapse&) = default;
...
private:
  ...
  int readSCInt_(const hid_t &file_id,
                 const std::string &name);
  void readBounds_(const hid_t &file_id,
                   const std::string &name,
                   int size, Real &lo, Real &hi);
  void readSCDset_(const hid_t &file_id,
                   const std::string &name,
                   Spiner::DataBox &db);
  ...
  lTFromlRhoSie_(const Real lRho, const Real sie,
                 Real *lambda) const noexcept;
  PORTABLE_INLINE_FUNCTION void __attribute__((always_inline))
  getLogsFromRhoT_(const Real rho, const Real temp, Real *lambda,
			Real &lRho, Real &lT, Real &Ye) const noexcept {
    checkLambda_(lambda);
    lRho = lRho_(rho);
    lT = lT_(temp);
    Ye = lambda[Lambda::Ye];
  ...
  PORTABLE_INLINE_FUNCTION __attribute__((always_inline))
  void getLogsFromRhoSie_(const Real rho, const Real sie, Real *lambda,
			  Real &lRho, Real &lT, Real &Ye) const noexcept {
    lRho = lRho_(rho);
...
using EOS = Variant<IdealGas, Gruneisen, JWL,
                    DavisReactants, DavisProducts,
                    ScaledEOS<IdealGas>,
                    ShiftedEOS<IdealGas>,
                    ShiftedEOS<ScaledEOS<IdealGas>>,
                    RelativisticEOS<IdealGas>
#ifdef SPINER_USE_HDF
                    ,SpinerEOSDependsRhoT,
                    SpinerEOSDependsRhoSie,
                    ScaledEOS<SpinerEOSDependsRhoT>,
                    ScaledEOS<SpinerEOSDependsRhoSie>,
                    ShiftedEOS<SpinerEOSDependsRhoT>,
                    ShiftedEOS<SpinerEOSDependsRhoSie>,
                    ...
                    // enthalpy weirdly.
                    StellarCollapse,
                    ScaledEOS<StellarCollapse>,
                    ShiftedEOS<StellarCollapse>,
                    ShiftedEOS<ScaledEOS<StellarCollapse>>,
                    RelativisticEOS<StellarCollapse>
#endif
                    >;
