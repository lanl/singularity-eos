...
#include <test/material_5phaseSesameSn.hpp>

namespace pte_test_3phaseSesameSn {
...
inline void set_trial_state(int n, RealIndexer &&rho, RealIndexer &&vfrac,
                            RealIndexer &&mfrac, RealIndexer &&sie) {

  Real vsum = 0.;
  for (int i = 0; i < NMAT; i++) {
    vfrac[i] = trial_vfrac[i];
    mfrac[i] = in_lambda[i][n];
    rho[i] = mfrac[i]*in_rho_tot[n] / vfrac[i];
