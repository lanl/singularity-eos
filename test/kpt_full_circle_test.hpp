...
inline void gibbsprintresults(int n, RealIndexer &&rho, RealIndexer &&temp, RealIndexer &&sie, RealIndexer &&gibbsrt, RealIndexer &&gibbsre){
      ...
      std::cout << "Internal energy: beta, gamma, hcp: \t" << sie[0] << ", "
                << sie[1] << ", " << sie[2] << std::endl;
      std::cout << "Gibbs(Density,Internal Energy): beta, gamma, hcp: \t" << gibbsre[0] << ", "
                << gibbsre[1] << ", " << gibbsre[2] << std::endl;
      std::cout << "Within 0.1 % from each other (0=False, 1=True): beta, gamma, hcp: \t " << isClose(gibbsrt[0],gibbsre[0], 1e-3) << ", "
                << isClose(gibbsrt[1],gibbsre[1],1e-3) << ", " << isClose(gibbsrt[2],gibbsre[2],1e-3) << std::endl;
      std::cout << "Within 0.1 % from Gibbs from LAP: beta, gamma, hcp: \t " << isClose(gibbsrt[0],out_gibbs0[n], 1e-3) << ", "
                << isClose(gibbsrt[1],out_gibbs1[n],1e-3) << ", " << isClose(gibbsrt[2],out_gibbs2[n],1e-3) << std::endl
                << std::endl;
}
...
template<typename RealIndexer>
inline void kptprintresults(int n, RealIndexer &&rho, RealIndexer &&vfrac, RealIndexer &&sie, RealIndexer &&press, RealIndexer &&temp){
      std::cout << "Trial number: " << n << std::endl;
